<?php
// $Id: email.php,v 1.10 2005/04/27 16:41:29 andrew Exp $
//  ------------------------------------------------------------------------ //
//  Author: Andrew Mills                                                     //
...
$id = $_GET['id'];
	...
	$xoopsOption['template_main'] = 'makale_email.html';
	...
	$sql = ("SELECT * FROM " . $xoopsDB->prefix("makale_main") . " WHERE id = $id LIMIT 1");
...
//print_r($formdata);
	...
	$myrow = $xoopsDB->fetchArray($result);
		...
		// grab prefs.
		$sql2 = ("SELECT * FROM " . $xoopsDB->prefix("makale_prefs") . " WHERE id=1 LIMIT 1");
		...
		$headers .= "X-mailer: makale - XOOPS v2 module at XOOPS_URL.\r\n";
		...
		// Replace {makale_URL} with URL to makale.
		$message = preg_replace("/{makale_URL}/", XOOPS_URL . "/modules/makale/makale.php?id=" . $emailsend['id'], $message);
